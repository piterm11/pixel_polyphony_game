...
function Lobby({match}) {
	...
	const getReady = async(data) => {
		const res = await axios.put(`http://localhost:8000/api/player/${match.params.id}/`, data)
		console.log(res)
	}
			...
			<div className="App-header">
				<div className="lobby-container">
					<div className="instruments">
						...
						<div className = {`single-instrument ${isClicked ? 'single-instrument-clicked' : ''} ${lobby.available_instruments.includes("piano") && !isActive? '': 'deactive' }`} onClick={toggleCliclked}>
							<div style={single(piano)}>
								Piano
							</div>
						</div>
						<div className = {`single-instrument ${isClicked2 ? 'single-instrument-clicked' : ''} ${lobby.available_instruments.includes("flute") && !isActive? '': 'deactive' }`} onClick={toggleCliclked2}>
							<div style={single(flute)}>
								Flute
							</div>
						</div>
						<div className = {`single-instrument ${isClicked3 ? 'single-instrument-clicked' : ''} ${lobby.available_instruments.includes("saxophone") && !isActive? '': 'deactive' }`} onClick={toggleCliclked3}>
							<div style={single(saxophone)}>
								Saxophone
							</div>
						</div>
						<div className = {`single-instrument ${isClicked4 ? 'single-instrument-clicked' : ''} ${lobby.available_instruments.includes("drum") && !isActive? '': 'deactive' }`} onClick={toggleCliclked4}>
							<div style={single(drum)}>
								Drum
							</div>
						</div>
						<div className = {`single-instrument ${isClicked5 ? 'single-instrument-clicked' : ''} ${lobby.available_instruments.includes("trumpet") && !isActive? '': 'deactive' }`} onClick={toggleCliclked5}>
							<div style={single(trumpet)}>
								Trumpet
							</div>
						</div>
					</div>
